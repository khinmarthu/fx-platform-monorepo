...
const initialState = tradesAdapter.getInitialState({ loading: false, error: null, status: TradeStatus.idle });
...
const tradeSlice = createSlice({
    ...
    reducers: {
        ...
        tradeAdded: (state, action: PayloadAction<TradeOrder>) => tradesAdapter.addOne(state, action.payload),

        // overwrites existing collection entirely with an array of incoming trades
        tradesBulkLoaded: (state, action: PayloadAction<TradeOrder[]>) => tradesAdapter.setAll(state, action.payload),

        // removes a single trade from the state via its id string
        tradeRemoved: (state, action: PayloadAction<string>) => tradesAdapter.removeOne(state, action.payload),

        // updates specific properties of a trade matching a specific target ID
        tradeUpdated: (state, action: PayloadAction<{ id: string; changes: Partial<TradeOrder> }>) => tradesAdapter.updateOne(state, action.payload),
...
export const { tradeAdded, tradesBulkLoaded, tradeRemoved, tradeUpdated, setLoading } = tradeSlice.actions;
